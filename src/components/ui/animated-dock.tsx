...
export const AnimatedDock = ({ className, items }: AnimatedDockProps) => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "flex h-20 items-end gap-5 rounded-2xl bg-secondary border border-primary/10 shadow-md px-6 pb-3",
        className,
      )}
    >
...
export const DockItem = ({ mouseX, children }: DockItemProps) => {
  ...
  const widthSync = useTransform(distance, [-150, 0, 150], [48, 96, 48]);
  ...
  const iconScale = useTransform(width, [48, 96], [1, 1.5]);
  ...
  return (
    <motion.div
      ref={ref}
      style={{ width }}
      className="aspect-square w-12 rounded-full bg-primary text-secondary-foreground flex items-center justify-center"
    >
      <motion.div
